# SVS_3DRooms

## Description
3D rooms plugin / MOD for `Samabake Scramble` / `サマバケ！すくらんぶる`. Moves the camera and perspective during HScenes so it "takes place" in the 3D maps.

## Prerequisites
- [BepInEx Unity Il2Cpp](https://github.com/BepInEx/BepInEx)

#### OR
- [SVS-HF_Patch](https://github.com/ManlyMarco/SVS-HF_Patch)

## Installation
1. Install prerequisites
2. Download from releases
3. Extract into game folder (.DLL file should be in `/GAME_FOLDER/BepInEx/plugins/`)

## Features
- Toggle 3D rooms perspective on/off if [SVS_BepisPlugins](https://github.com/IllusionMods/BepisPlugins) and [ConfigurationManager](https://github.com/BepInEx/BepInEx.ConfigurationManager) is installed

## Notes
- The other camera post processing will be applied to the HScene camera if enabled (when they normally wouldn't)